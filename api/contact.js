...
export default async function handler(req, res) {
    ...
    if (turnstileSecretKey) {
        if (!turnstileToken) {
            return res.status(400).json({ error: 'Spam check is required.' });
        }
        ...
        try {
            ...
            if (!verification.success) {
                return res.status(400).json({ error: 'Spam check failed. Try again.' });
            }
        } catch (error) {
            console.error('Turnstile verification error:', error);
            return res.status(502).json({ error: 'Spam protection is unavailable right now.' });
        }
    }
