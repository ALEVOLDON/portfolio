...
const AvatarStatic = ({ profile }) => {
    const avatarSrc = profile?.avatar_url || '/avatar-320.jpg';
    const avatarAlt = profile?.name || 'Vladimir Rybalsky';

    return (
        <div className="relative inline-block mb-8 group select-none">
            <div className="absolute -inset-2 bg-gradient-to-r from-cyber-cyan via-cyber-purple to-cyber-cyan rounded-full blur-md opacity-30 group-hover:opacity-70 transition duration-500 animate-pulse" />
            ...
            <div className="relative w-64 h-64 rounded-full overflow-hidden border border-cyber-cyan/30 bg-cyber-black/80 flex items-center justify-center">
                ...
                <div className="absolute top-2 left-1/2 transform -translate-x-1/2 font-mono text-[7px] text-cyber-cyan/60 tracking-widest z-10 uppercase pointer-events-none">
                    GLB_3D_SECURE
                </div>
